...
func TestFileLogWriter(t *testing.T) {
	...
	w := NewFileLogWriter(testLogFile, MAX_BACK_UP, false, false)
	if w == nil {
...
func TestXMLLogWriter(t *testing.T) {
	...
	w := NewXMLLogWriter(testLogFile, MAX_BACK_UP, false, false)
	if w == nil {
		t.Fatalf("Invalid return: w should not be nil")
	}
	defer os.Remove(testLogFile)
...
func TestLogOutput(t *testing.T) {
	...
	l.AddFilter("file", FINEST, NewFileLogWriter(testLogFile, MAX_BACK_UP, false, false).SetFormat("[%L] %M"))
	defer os.Remove(testLogFile)
...
func BenchmarkFileLog(b *testing.B) {
	sl := make(Logger)
	b.StopTimer()
	sl.AddFilter("file", INFO, NewFileLogWriter("benchlog.log", MAX_BACK_UP, false, false))
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		sl.Log(WARNING, "here", "This is a log message")
	}
